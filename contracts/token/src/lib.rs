...
#[derive(BorshDeserialize, BorshSerialize, PanicOnDefault)]
pub struct Staking{
    pub voice_power: HashMap<AccountId, u32>,
}

impl Staking{
    pub fn stake(&mut self, quantity: u32)
    {
        //transfer
        self.voice_power.insert(env::signer_account_id(), quantity);
    }
    pub fn unstake(&mut self)
    {
        self.voice_power.remove(&env::signer_account_id());
    }
    pub fn get(&mut self, user: AccountId) ->bool
    {
